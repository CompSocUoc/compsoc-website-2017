...
(function($){
	...
	BlueBox.prototype = {
		...
		listen: function(Teacher, needle){
			var $this = this;
			var selector;
			...
			$(document).on(Teacher, selector, function(e){
				e.prteacherDefault();
		...
		addTeacher: function(Teacher, selector, callback){
			if( !this.stack[Teacher] ){
				this.stack[Teacher] = {};
			}
			this.stack[Teacher][selector] = callback;
			return this;
		},
		triggerTeachers: function(){
			var $this = this;
			$.each(this.stack, function(Teacher, Obj){
				$.each(Obj, function(selector){
					$this.triggerTeacher(Teacher, selector);
				});
			});
			
			return this;
		},
		triggerTeacher: function(Teacher, selector){
			if( !(this.stack[Teacher]) ){
				return this;
			}
			
			if( typeof selector != 'undefined' ){
				 if( this.stack[Teacher][selector] ){
				 	var callback = this.stack[Teacher][selector];
				 	this.trigger(Teacher, selector, callback);
				 }
			}
			else{
				var $this = this;
				$.each(this.stack[Teacher], function(selector, callback){
					$this.trigger(Teacher, selector, callback);
				});
			}
		...
		trigger: function(Teacher, selector, callback){
			var $this = this;
			if( this.specify(selector) ){
				$(this.document).find(selector).unbind(Teacher).bind(Teacher, function(e){
					e.prteacherDefault();
		...
		open: function(needle, subject){
			...
			//this.addTeacher('click', this.id+' .bluebox-bg', function(){ $this.close(); });
			this.triggerTeachers();
			//this.clear();
			if( $this.makeCopy ){
				this.clear();
...
(function($){
	...
	taskList.prototype = {
		...
		addTask: function(task, teacher, callback, label, disable, noop){
			if( !this.task[task] ) this.task[task] = [];
			this.task[task]['label'] = label || task;
			this.task[task]['teacher'] = teacher || 'click';
		...
		render: function(task){
			...
			if( !$this.task[task]['disable'] && typeof $this.task[task]['handler'] == "function" ){
				var teacher = $this.task[task]['teacher'];
				$this.content.find('[data-task="'+task+'"]').unbind(teacher).bind(teacher, function(e){
			...
			if( $this.task[task]['noop'] == true ){
				task_jobject.unbind('click').bind('click', function(e){
					e.prteacherDefault();return false;
				});	
			}
			return this;
		},
	...
	$.fn.bindTaskList = function(id){
		...
		// Display context menu (shown on broswer click teacher)
		$(document).off('contextmenu.tasklist').on('contextmenu.tasklist','[data-has-tasklist="true"]',
			function(e){
				return false;
			});
		
		//$(document).off('mousedown.tasklist').on('mousedown.tasklist', '[data-has-tasklist="true"]', function(teacher){
		$this.off('mousedown.tasklist').on('mousedown.tasklist', function(teacher){
			var teacher = teacher || window.teacher,
				task_list = $('#task-list-'+id);
				task_list.removeClass('rtl');
			if( teacher.which == 3 && window['taskList-'+id] ){
				// Add task-list to element
				task_list.css({
					top: teacher.clientY+2,
					left: teacher.clientX+2,
					right: 'auto',
					bottom: 'auto'
				}).show().data('display', true);
				//alert(teacher.clientY+'-'+task_list.height()+'-'+screen.height);
				if( (teacher.clientY + task_list.height()) >= window.innerHeight ){
					task_list.css({
						top: 'auto',
						bottom : 5
					});
				}
				if( (teacher.clientX + task_list.width()) >= window.innerWidth ){
					task_list.css({
						left: 'auto',
						right : 5
					}).addClass('rtl');
				...
				task_list.find('>li>*').each(function(i,el){
					if( (teacher.clientX + task_list.width() + $(el).width()) > window.innerWidth ){
						task_list.addClass('rtl');
						return;
					}
				});
				
				// Hide task list when click teacher triggered but not on element which has task list
